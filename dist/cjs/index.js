...
function getDefaultAssetPathInfo(fullFilePath, projectRootDir) {
    ...
    const pathRelativeToProject = path.relative(projectRootDir, p);
    ...
    return {
        ...
        async resolveId(id, importer, { assertions }) {
            ...
            if (id.startsWith(DATA_PREFIX) || (!!include && filter(id))) {
                ...
                let importerDir = importer ? path.dirname(importer) : projectDir;
                let inputFilePath = url.pathname.startsWith("~/") ? path.join(projectDir, url.pathname.substring(2)) :
                    url.pathname.startsWith("~") ? path.join(projectDir, url.pathname.substring(1)) :
                        path.isAbsolute(url.pathname) ? url.pathname :
                            path.join(importerDir, url.pathname);
...
export default data;`;
                    ...
                    return `
const data = ${JSON.stringify(path.relative(info.outputDirectory, info.outputFilePath))};
export default data;`;
                }
                else {
                    return `
...
export default data;`;
        ...
        async buildEnd() {
            ...
            await Promise.all([...infoByImport].map(async ([, info]) => { info.rawData = await promises.readFile(info.inputFilePath); }));
...
exports.getDefaultAssetPath = getDefaultAssetPath;
exports.getDefaultAssetPathInfo = getDefaultAssetPathInfo;
